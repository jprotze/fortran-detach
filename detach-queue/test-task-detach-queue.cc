...
void queue_detach(const int* a, int* b, omp_event_handle_t event, std::chrono::duration<R,D> workLoad) {
  WorkItem work{event, std::chrono::steady_clock::now() + workLoad, a, b, ++workId};
...
void run(void){
  while(running){
    {
      std::unique_lock<std::mutex> lck(workQueueMtx);
      while(!workQueue.empty() && workQueue.front().duetime < std::chrono::steady_clock::now() )
      {
